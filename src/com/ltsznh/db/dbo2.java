...
public class dbo2 {
	...
	public static ResultData executeParamQuerySilence(String sqlString, HashMap<String, Object> param,
		...
		try {
			...
			ServerLog.err(param.get("id").toString(), e);
	...
	public static ResultData executeParamQuery(String sqlString, HashMap<String, Object> param, String[] sqlParam) {
		...
		try {
			conn = getConn();
			ServerLog.log("DB", param.get("id").toString(), sqlString);
			StringBuffer paramBuffer = new StringBuffer();
			pStmt = conn.prepareStatement(sqlString);
			if (sqlParam != null) {
				for (int i = 0; i < sqlParam.length; i++) {
				...
				ServerLog.log("DB", param.get("id").toString(), "参数：" + paramBuffer.toString());
			}
			long sTime = System.currentTimeMillis();
			rs = pStmt.executeQuery();
			long eTime = System.currentTimeMillis();
			...
			setResultSet(result, rs);
			result.setTime((eTime - sTime));
			ServerLog.log("DB", param.get("id").toString(),
	...
	public static ResultData executeParamQueryPaging(HashMap<String, Object> param, String[] sqlParam) {
		...
		try {
			...
			ServerLog.log("DB", param.get("id").toString(), sqlRowsString);
			...
			ServerLog.log("DB", param.get("id").toString(), "总共： " + result.size() + "行" + " 耗时： " + (eTime - sTime)
	...
	public static ResultData getUserRW(HashMap<String, Object> param) {
		ResultData result;
		try {
			ServerLog.log("DB", param.get("id").toString(), "获取用户读写权限");
			...
			result.setMessage("ID:" + param.get("id").toString());
			result.setStatus(false);
			ServerLog.err("getUserRW", param.get("id").toString(), e);
		}
		return result;
	}
	...
	public static void updateLog(HashMap<String, Object> param, String sqlString, StringBuffer paramBuffer, long rows) {
		...
		try {
			...
			// TODO: handle exception
			ServerLog.err("updateLog", param.get("id").toString(), e);
		}
		// ServerLog.log(
		// "TTTTT",
		// "",
	...
	public synchronized static Connection getConn() throws Exception {
		...
		try {
			...
			// TODO: handle exception
			ServerLog.err("DB", "", e.getLocalizedMessage());
		} finally {
	...
	public synchronized static Statement getStatement(Connection conn) throws Exception {
		...
		try {
			...
			if (stmt == null) {// 创建链接状态
				ServerLog.log("Server", "getStatement", "stmt为空，生成状态！");
				try {
					// stmt =
					...
					try {
						ServerLog.err("Server", "getStatement", e);
						ServerLog.log("Server", "getStatement", "创建链接失败，重新生成连接...");
						stmt = conn.createStatement();
					} catch (Exception ee) {
						ServerLog.err("Server", "getStatement", ee);
					}
				}
			}
		}
	...
	private static void release(ResultSet rs) {
		if (rs != null) {
			try {
				...
				ServerLog.err("Server", "release rs", e);
			}
			rs = null;
		}
	}
